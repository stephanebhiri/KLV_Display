 ...
 *
 * NOTE: GStreamer 1.20's webrtcbin works best when the entire pipeline is created
 * with gst_parse_launch, letting GStreamer handle the pad negotiation automatically.
 */
static gboolean create_pipeline(void) {
    ...
    if (gateway->shm_input) {
        ...
        // NOTE: webrtcbin 1.20 has linking issues - we create the pipeline in two steps
        pipeline_str = g_strdup_printf(
            "shmsrc socket-path=%s is-live=true do-timestamp=true "
            "! video/x-raw,format=I420,width=%d,height=%d,framerate=%d/1 "
            "! queue max-size-buffers=10 leaky=downstream "
            "! videoconvert "
            "! vp8enc name=encoder deadline=1 cpu-used=8 target-bitrate=%d000 keyframe-max-dist=30 "
            "! rtpvp8pay name=pay pt=96 mtu=1200",
            gateway->shm_path,
            gateway->width,
            gateway->height,
            gateway->fps,
    ...
    g_printerr("Creating encoder pipeline: %s\n", pipeline_str);

    // Create encoder pipeline (without webrtcbin)
    GstElement *encoder_pipeline = gst_parse_launch(pipeline_str, &error);
    g_free(pipeline_str);
    ...
    // Create the main pipeline as a bin
    gateway->pipeline = gst_pipeline_new("webrtc-pipeline");

    // Create webrtcbin separately
    gateway->webrtcbin = gst_element_factory_make("webrtcbin", "webrtcbin");
    if (!gateway->webrtcbin) {
        g_printerr("Failed to create webrtcbin\n");
        send_error("Failed to create webrtcbin");
        gst_object_unref(encoder_pipeline);
        return FALSE;
    }

    // Configure webrtcbin
    g_object_set(gateway->webrtcbin,
        "bundle-policy", 3,  // max-bundle
        "stun-server", gateway->stun_server,
        NULL);

    // Add encoder pipeline (as a bin) and webrtcbin to main pipeline
    gst_bin_add_many(GST_BIN(gateway->pipeline),
        encoder_pipeline, gateway->webrtcbin, NULL);

    // Get the rtpvp8pay element's src pad
    GstElement *pay = gst_bin_get_by_name(GST_BIN(encoder_pipeline), "pay");
    if (!pay) {
        g_printerr("Failed to get pay element\n");
        send_error("Failed to get pay element");
        return FALSE;
    }
    ...
    if (!src_pad) {
        g_printerr("Failed to get src pad from pay\n");
        send_error("Failed to get src pad from pay");
        return FALSE;
    }

    // Create a ghost pad on the encoder bin so we can link it
    GstPad *ghost_src = gst_ghost_pad_new("src", src_pad);
    gst_pad_set_active(ghost_src, TRUE);
    gst_element_add_pad(encoder_pipeline, ghost_src);
    gst_object_unref(src_pad);

    // Now link encoder bin to webrtcbin
    // Use gst_element_link which will auto-request the sink pad
    if (!gst_element_link(encoder_pipeline, gateway->webrtcbin)) {
        g_printerr("Failed to link encoder to webrtcbin\n");
        send_error("Failed to link encoder to webrtcbin");
        return FALSE;
    }

    g_printerr("Pipeline created and linked successfully\n");
